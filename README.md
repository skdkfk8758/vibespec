...
- **Error Knowledge Base** — 에러 지식을 마크다운으로 축적, Obsidian vault 양방향 동기화, 자동 검색/기록
...
# 에러 지식 베이스
vs error-kb search <query> [--tag <tag>] [--severity <level>] [--with-obsidian]
...
vs error-kb sync [--import] [--dry-run]     # Obsidian vault 양방향 동기화
# --vault <name> 옵션으로 모든 error-kb 서브커맨드에 vault 지정 가능

# 설정
vs config set <key> <value>                  # 설정값 저장 (예: obsidian.vault)
...
└── src/                        # CLI 소스
    ...
    └── core/
        ├── config.ts             # 설정 관리 (Obsidian vault 경로 등)
        ├── adapter/
