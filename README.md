...
- **CLI (`vp`)** — 터미널에서 직접 플랜/태스크를 관리하는 CLI (스킬에서도 자동 호출)
...
스킬 내부에서 `vp ... --json`으로 자동 호출되며, 터미널에서도 직접 사용할 수 있습니다.
...
# 플랜 관리
vp plan list [--status active] [--branch main]
vp plan create --title "새 기능 구현" [--spec "..."] [--summary "..."]
vp plan show <plan_id>
vp plan complete <plan_id>
vp plan approve <plan_id>
vp plan archive <plan_id>
vp plan update <plan_id> [--title "..."] [--spec "..."] [--summary "..."]
vp plan delete <plan_id>

# 태스크 관리
vp task create --plan <plan_id> --title "..." [--spec "..."] [--acceptance "..."] [--depends-on "id1,id2"]
vp task show <task_id>
vp task next <plan_id>
vp task update <task_id> <status> [--has-concerns] [--impl-status DONE]
vp task edit <task_id> [--title "..."] [--spec "..."] [--acceptance "..."]
vp task block <task_id> [--reason "..."]
vp task delete <task_id>

# 컨텍스트
vp context resume [--session-id <id>]
vp context save --summary "..."

# 통계 & 인사이트
vp stats [plan_id]
vp history <entity_type> <entity_id>
vp insights [--scope all|blocked_patterns|duration_stats|success_rates]
```
