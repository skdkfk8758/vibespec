...
```
충돌 발견 시 **하위 문서 수정을 기본 제안**합니다. 상위 문서 변경이 필요하면 Locked tier로 분류합니다.
...
3. **Locked 변경 제안** (구조화된 사유 입력):
   - **Step 1**: AskUserQuestion으로 변경 내용 표시 + 카테고리 선택:
     - "비즈니스 변경" — 사업 방향/전략 변경에 의한 수정
     - "기술 불가" — 기존 기술적 제약으로 변경 필요
     - "보안 재평가" — 보안 요구사항 변경
     - "기타" — 위에 해당하지 않는 사유
   - **Step 2**: 사유 텍스트 입력 요청 (최소 50자)
     - 50자 미만: "사유가 너무 짧습니다 ({현재 글자수}자). 최소 50자 이상 입력하세요." + 재입력
     - 빈 입력: 변경 차단 + "Locked 변경은 사유 없이 적용할 수 없습니다." 안내
   - **Step 3**: 카테고리 + 사유 확인 후 적용
   - skeleton_changes에 기록: (approved_by: 'user', category: 카테고리, reason: 사유)
