...
3. **디자인 검증 체크리스트**
   ...
   Browser-Control 검증 후, 각 페이지/컴포넌트에 대해 다음 디자인 검증을 수행하세요:

   **시각적 일관성 (Visual Consistency)**
   - [ ] **간격/정렬**: 요소 간 spacing이 일관적인가 (config.design.grid_unit (기본값: 8)px grid 단위 등), 수직/수평 정렬이 맞는가
   - [ ] **폰트**: 동일 역할의 텍스트에 동일 font-family, font-size, font-weight 적용 여부
   - [ ] **색상**: 브랜드 색상 일관 적용, 하드코딩된 색상값 대신 디자인 토큰/변수 사용 여부
   - [ ] **아이콘/이미지**: 깨진 이미지 없음, 아이콘 크기/스타일 일관성

   **반응형 레이아웃 (Responsive Layout)**
   - [ ] 뷰포트 크기 변경 시 레이아웃이 깨지지 않는가 (config.design.responsive_breakpoints (기본값: [1280, 768, 375]) 기준)
   - [ ] 요소 겹침, overflow, 잘림 현상 없는가
   - [ ] 텍스트 truncation이 적절하게 처리되는가

   **AI Slop 탐지** (config.design.ai_slop_detection (기본값: true)이 true일 때만 실행)
   - [ ] **과도한 그라디언트**: 불필요하게 복잡한 `linear-gradient`, `radial-gradient` 남용
   - [ ] **제네릭 아이콘**: placeholder 아이콘, 맥락과 무관한 장식적 아이콘
   - [ ] **불일치 간격**: 같은 리스트/그리드 내에서 항목 간 spacing이 제각각
   - [ ] **무의미한 장식**: 기능과 무관한 과도한 그림자, 애니메이션, 장식 요소
   - [ ] **제네릭 텍스트**: Lorem ipsum, placeholder 텍스트가 남아있는지

   **DESIGN.md 참조 검증** (프로젝트 루트에 DESIGN.md가 존재하는 경우)
   - [ ] DESIGN.md에 명시된 색상 팔레트와 실제 사용 색상 일치 여부
   - [ ] DESIGN.md에 명시된 타이포그래피 규칙 준수 여부
   - [ ] DESIGN.md에 명시된 컴포넌트 스타일 가이드 준수 여부
   - DESIGN.md가 없으면 이 항목은 SKIP 처리

   **디자인 검증 판정:**
   - 모든 체크리스트 통과 → 기존 결과 유지
   - AI slop 또는 시각적 불일치 발견 → 기존 결과를 최소 **WARN**으로 격상
   - 심각한 레이아웃 깨짐 → **FAIL**

4. **결과 판정**
   - **PASS**: 기대 요소가 존재하고, 인터랙션이 정상 동작하고, 콘솔 에러 없음, 디자인 검증 통과
   - **WARN**: 기대 요소가 존재하지만 콘솔 경고 있음, 또는 스타일/디자인 이슈
   - **FAIL**: 기대 요소 미존재, 인터랙션 실패, 콘솔 에러 발생, 또는 심각한 레이아웃 깨짐

5. **DB 기록**
   - 시나리오 상태 업데이트: `vs qa scenario update <scenario_id> <status> --json`
   - evidence에 스크린샷 경로, 콘솔 로그, 디자인 검증 결과 포함
   - FAIL/WARN인 경우 finding 생성 전 검증:
   ...
   - FAIL/WARN인 경우 finding 생성:
     ...
     vs qa finding create <run_id> \
       ...
       --category <bug|regression|ux_issue|design_issue> \
       --related-scenario <scenario_id>
     ```
   - 디자인 이슈 finding의 category는 `design_issue`를 사용하세요

6. **Browser-Control 불가 시**
