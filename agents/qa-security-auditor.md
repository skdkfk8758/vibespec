...
1. **변경 파일 로드**
   - config.security.scope (기본값: changed_files) 설정에 따라 검사 범위를 결정하세요
...
config.security.owasp_skip (기본값: []) 목록의 카테고리는 건너뛰세요. 나머지 카테고리를 순서대로 검사하세요:

1. **A01: Broken Access Control** — 접근 제어 누락/우회
2. **A02: Cryptographic Failures** — 약한 암호화, 시크릿 노출
3. **A03: Injection** — SQL/XSS/Command 인젝션
4. **A04: Insecure Design** — 하드코딩된 비밀값, 에러 노출, 클라이언트 측 검증 의존
5. **A05: Security Misconfiguration** — CORS 와일드카드, 디버그 모드, 환경변수 노출
6. **A06: Vulnerable Components** — 알려진 취약 의존성 (package.json/requirements.txt 없으면 SKIP)
7. **A07: Auth Failures** — 인증 메커니즘 취약점
8. **A08: Software Integrity** — 검증 없는 외부 스크립트, 안전하지 않은 역직렬화
9. **A09: Security Logging** — 보안 로깅 누락, 민감정보 로깅
10. **A10: SSRF** — 사용자 입력 기반 URL 요청 (서버 사이드 코드 없으면 SKIP)
...
### Phase 4.5: Custom Rules 실행 (Phase 0에서 config 로딩 후)
...
### Phase 4.6: Finding 생성 전 검증 (config 기반)
...
- 테스트 파일의 취약점은 config.security.severity_overrides.test_files (기본값: downgrade_one) 설정에 따라 severity를 조정하세요
