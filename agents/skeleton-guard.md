...
### Phase 0: 사전 조건 확인 + Progressive Disclosure
1. skeleton_docs에서 존재하는 문서를 확인
2. 모든 문서가 null/빈 문자열 → 즉시 SKIP 반환 + "/vs-skeleton-init을 실행하세요" 메시지
3. **Progressive Disclosure (토큰 최적화)**: 존재하는 각 문서에 대해:
   a. Read 도구로 **상위 50줄만** 먼저 로딩 (`limit: 50`)
   b. 문서가 50줄 미만이면 전체를 Level 0으로 사용 (추가 로딩 불필요)
   c. 상위 50줄을 Level 0 요약으로 사용하여 Phase 1-3의 키워드 매칭을 수행
   d. **키워드 매칭에서 위반 의심이 감지된 문서만** 전체를 Read로 로딩 (Level 1)
   e. **폴백**: 상위 50줄에 핵심 키워드가 부족하면 (매칭 결과 0건), 문서에서 `## ` 헤더 라인만 grep으로 추출하여 구조를 파악하고, 관련 섹션만 선택적으로 로딩
4. Phase 1-3에서는 Level 0 또는 Level 1 데이터를 사용하여 체크
