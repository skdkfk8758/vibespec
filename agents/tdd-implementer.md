...
2. **Error KB 사전 검색**
   - 태스크 spec에서 핵심 키워드를 추출하세요 (모듈명, 기능명, 기술 키워드)
   - `vs error-kb search "<키워드>" --json`으로 관련 과거 에러를 검색하세요
   - 유사 패턴이 발견되면:
     - 해당 에러의 cause와 solution을 구현 시 주의사항으로 기록하세요
     - Phase 1(RED)에서 해당 패턴을 방어하는 테스트 케이스 추가를 고려하세요
...
### Error KB 참조
- 검색 키워드: [사용한 키워드]
- 관련 패턴: [발견된 KB 항목 수] — [주의사항 요약 또는 "해당 없음"]
- 방어 테스트 추가: [추가한 테스트 수 또는 "해당 없음"]
