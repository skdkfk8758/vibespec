...
- **mode**: `full` (전체 QA) | `incremental` (마지막 QA 이후 변경분만) | `targeted` (특정 태스크/영역) | `visual` | `design-verification` | `delta` (이미 검증된 시나리오 제외)
...
delta 모드에서는 이미 검증된 시나리오를 제외하고 미검증 영역만 시나리오를 생성합니다.

1. 기존 검증 시나리오 조회:
   `vs --json qa scenario list <이전 run_id>` 또는 전체 플랜의 시나리오에서
   source IN ('seed', 'shadow', 'wave') AND status = 'pass'인 시나리오를 수집

2. 제외 로직:
   - 이미 PASS한 시나리오의 title/description과 유사한 새 시나리오 생성 억제
   - related_tasks가 동일하고 카테고리가 같은 시나리오는 중복으로 간주

3. 새 시나리오만 생성:
   - 코드 변경이 있었지만 seed/shadow에서 커버되지 않은 영역
   - 새로 추가된 태스크의 AC
   - cross-task 통합 시나리오 (개별 태스크 검증으로는 커버 불가)

4. 결과:
   - 생성 시나리오 수가 full 모드 대비 50-80% 감소 예상
   - "Delta 모드: {N}개 기존 시나리오 제외, {M}개 신규 시나리오 생성" 메시지

#### Seed 시나리오 중복 방지 (전체 모드 공통)

시나리오 생성 전에 기존 seed 시나리오를 먼저 조회하여 중복을 방지합니다:

1. `vs --json qa scenario list <run_id>`로 현재 run의 시나리오를 조회하세요 (없으면 스킵)
2. 이전 QA Run이 있으면 `vs --json qa scenario list <이전 run_id>`로 seed 시나리오(source='seed')를 조회하세요
3. 수집된 seed 시나리오 목록에서:
   - 각 seed 시나리오의 `related_tasks`와 `category`를 기록하세요
   - 각 seed 시나리오가 커버하는 AC 번호를 title/description에서 추출하세요
4. 아래 시나리오 생성 시:
   - seed가 이미 커버하는 AC + 카테고리 조합은 **시나리오 생성을 스킵**하세요
   - seed가 커버하지 않는 AC만 신규 시나리오로 생성하세요
   - integration/flow 등 seed에서 생성되지 않는 cross-task 시나리오는 항상 생성하세요
5. 중복 방지 결과를 기록하세요:
   ```
   Seed 중복 방지: {N}개 seed 시나리오 발견, {M}개 AC 이미 커버됨, {K}개 신규 시나리오 생성
   ```

5개 카테고리로 시나리오를 생성합니다. config.modules에서 비활성화된 카테고리는 시나리오 생성에서 제외하세요. 각 시나리오는 다음 형식을 따릅니다:
...
3. **리스크 스코어 계산**
   ...
   scenario_risk = Σ(failed시나리오 × severity_weight) / total_scenarios
     severity_weight: critical=config.severity_weights.critical (기본값: 0.4), high=config.severity_weights.high (기본값: 0.3), medium=config.severity_weights.medium (기본값: 0.2), low=config.severity_weights.low (기본값: 0.1)
...
- 리스크 등급: {GREEN|YELLOW|ORANGE|RED}
  - GREEN (0.0–config.risk_thresholds.green (기본값: 0.2)): 안전 — 릴리즈 가능
  - YELLOW (config.risk_thresholds.green–config.risk_thresholds.yellow (기본값: 0.2–0.5)): 주의 — 발견 이슈 검토 필요
  - ORANGE (config.risk_thresholds.yellow–config.risk_thresholds.orange (기본값: 0.5–0.8)): 위험 — 수정 필요
  - RED (config.risk_thresholds.orange–1.0 (기본값: 0.8–1.0)): 심각 — 수정 플랜 필수
