...
- 전체 태스크를 개별 조회하지 마세요. `vp plan show {plan_id}` CLI 명령 하나로 트리 구조를 가져오세요.
- 사용자가 선택한 항목만 `vp task show {task_id}`로 상세 조회하세요.
...
주요 명령:
- `vp plan list` — 플랜 목록
- `vp plan show <plan_id>` — 플랜 상세 + 태스크 트리 (한 번의 호출로 전체 구조 확인)
- `vp task show <task_id>` — 개별 태스크 상세 (spec, acceptance 포함)
- `vp task create --plan <id> --title <t> --spec <s> --acceptance <a>` — 태스크 생성
- `vp task update <task_id> <status>` — 태스크 상태 변경

## Steps

1. **플랜 선택**
   - 인자로 plan_id가 주어지면 해당 플랜을 사용하세요
   - 없으면 `vp plan list`로 active 상태인 플랜 목록을 가져와서 선택하게 하세요
   - 플랜이 없으면 `/vs-plan`으로 먼저 플랜을 만들도록 안내하세요

2. **플랜 트리 조회 (단일 호출)**
   - `vp plan show {plan_id}` 한 번만 호출하여 전체 태스크 트리를 가져오세요
...
4. **선택에 따른 상세 조회 및 파일 생성**
   ...
   **B) 태스크 선택 수정 시:**
   ...
   - 선택된 태스크만 `vp task show {task_id}`로 상세 조회하세요
   ...
   **C) 전체 리뷰 선택 시:**
   - 모든 태스크를 `vp task show`로 조회 (이때만 전체 조회)
