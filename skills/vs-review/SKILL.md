...
4. **선택에 따른 상세 조회 및 파일 생성**
   ...
   **YAML 프론트매터 변조 방지:**
   - 임시 파일 상단의 YAML 프론트매터 블록(`---` ~ `---`) 바로 아래에 아래 주석을 포함하세요:
     ```markdown
     <!-- ⚠️ 주의: 위 메타데이터(plan_id, task_id 등)는 수정하지 마세요. 변경 시 DB 반영이 실패합니다. -->
     ```
   - 파싱 시 프론트매터 필드(plan_id, title 외의 task_id 등)가 원본과 달라졌으면 해당 변경을 무시하고 사용자에게 경고하세요
...
5. **에디터 열기**
   ...
   **에디터 타임아웃 처리:**
   - 10분 타임아웃에 도달하여 에디터가 자동 종료된 경우:
     1. 임시 파일은 삭제하지 않고 그대로 유지하세요
     2. 사용자에게 임시 파일 경로를 안내하세요: "타임아웃이 발생했습니다. 임시 파일이 `.moai/tmp/vs-review-{plan_id}.md`에 남아 있습니다."
     3. 수동 편집 완료 후 `/vs-review`를 다시 실행하면 기존 임시 파일을 감지하여 이어서 처리할 수 있도록 안내하세요
   - 재실행 시 해당 경로에 임시 파일이 이미 존재하면:
     → "이전 리뷰 세션의 임시 파일이 있습니다. 이 파일을 이어서 반영할까요, 새로 시작할까요?" 라고 확인하세요
