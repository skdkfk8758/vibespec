...
### 다음 단계
- /vs-deploy — 배포 설정이 구성되어 있다면 프로덕션 배포를 진행하세요
- /vs-deploy-setup — 배포 설정이 없다면 먼저 배포 환경을 구성하세요
- /vs-canary — 배포 후 헬스체크로 서비스 상태를 확인하세요
```

## Post-Release Checkpoint: 배포 자동 제안

Phase 7 결과 보고 후, 배포 설정 여부를 확인하여 자동으로 다음 단계를 제안합니다:

1. `vs config get deploy.command`로 배포 설정 확인
2. **설정이 있는 경우:**
   ```
   배포 설정이 감지되었습니다 (플랫폼: {deploy.platform}).
   릴리즈된 v{next}를 바로 배포할까요?
   - Y: /vs-deploy 실행
   - N: 나중에 수동으로 배포
   ```
3. **설정이 없는 경우:**
   ```
   배포 설정이 아직 구성되지 않았습니다.
   프로덕션 배포를 설정하려면 /vs-deploy-setup을 사용하세요.
   ```
