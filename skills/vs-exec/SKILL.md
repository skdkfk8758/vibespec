...
| 검증 | verifier + codex-review | verifier 에이전트 (codex-review 생략) |
...
8. **태스크 루프**
   ...
   **c. 검증 (기본 모드: verifier 에이전트 디스패치)**
   ...
   > **참고**: codex-review는 vs-exec의 속도 우선 특성상 생략합니다. 독립 코드 리뷰가 필요하면 PR 리뷰를 활용하세요.
...
- **검증 에이전트 사용**: 기본 모드에서는 verifier 에이전트를 디스패치하여 독립 검증을 수행합니다. 구현(tdd-implementer)과 코드 리뷰(codex-review)는 서브에이전트 없이 직접 수행합니다. `--inline` 모드에서는 검증도 직접 수행합니다.
