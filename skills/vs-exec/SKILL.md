...
2. **활성 플랜 확인**
   - `vp_plan_list`로 active/approved 플랜을 조회하세요
   - 여러 개면 `AskUserQuestion`으로 선택받으세요
   - 없으면 `/vs-plan`으로 안내하세요

3. **플랜 전체 로드**
   - `vp_plan_get(plan_id)`으로 spec + 전체 태스크 트리를 가져오세요
...
8. **태스크 루프**
   ...
   **a. 태스크 시작**
   ...
   - `vp_task_update(task_id, 'in_progress')`로 상태 변경
   ...
   **e. 태스크 완료 처리**
   - PASS: `vp_task_update(task_id, 'done', metrics)` — 다음 태스크로 진행
   - WARN: 간략히 사용자에게 알리고 `has_concerns: true`로 기록 후 계속 진행 (최종 리포트에서 모아서 보고)
   - FAIL: 1회 자동 수정 시도 → 재검증 → 여전히 FAIL이면 blocked 처리하고 사용자에게 보고

   **f. 진행 저장**
   - 3개 태스크마다 또는 FAIL 발생 시 `vp_context_save`로 진행 상황 저장
