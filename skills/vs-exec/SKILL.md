...
8. **태스크 루프**
   ...
   **WARN vs FAIL 구분 기준표:**
   ...
   | 구분 | FAIL (즉시 대응 필요) | WARN (기록 후 계속 진행) |
   |------|----------------------|------------------------|
   | 테스트 | 테스트 실패 (기존 또는 신규) | — |
   | 빌드 | 빌드 실패 | — |
   | Acceptance | AC 항목 명시적 미충족 | AC 항목 충족 여부 코드만으로 확인 불가 (수동 검증 필요) |
   | Lint | — | lint 경고 (에러가 아닌 warning 수준) |
   | 기존 코드 영향 | 기존 테스트 깨짐 | 기존 테스트 영향 없는 새 경고 발생 |
   | Scope | — | 범위 밖 파일 변경 (WARN 보고) |
   | 타입 체크 | 타입 에러 | 타입 경고 (strict 모드 관련 등) |

   > 판단이 모호한 경우 WARN으로 처리하고 종합 리포트에 상세 사유를 기록하세요. FAIL은 "이대로 진행하면 다음 태스크에 영향을 주는" 경우에만 적용하세요.
