...
vs-next/vs-pick은 태스크마다 tdd-implementer, verification, codex-review 에이전트를 디스패치하여 높은 품질을 보장합니다. 이 스킬은 서브에이전트를 사용하지 않으므로 컨텍스트 격리나 독립 리뷰의 이점이 없지만, 단일 세션에서 플랜을 빠르게 소화할 수 있습니다.
...
| 코드 리뷰 | codex-review 병렬 실행 | 셀프 리뷰 (편향 가능) |
...
2. **활성 플랜 확인**
   - Bash 도구로 `vp plan list --json` 명령을 실행하세요. active/approved 플랜을 조회하세요
   - 여러 개면 `AskUserQuestion`으로 선택받으세요
   - 없으면 `/vs-plan`으로 안내하세요

3. **플랜 전체 로드**
   - Bash 도구로 `vp plan show <plan_id> --json` 명령을 실행하세요. spec + 전체 태스크 트리를 가져오세요
...
8. **태스크 루프**
   ...
   **a. 태스크 시작**
   ...
   - Bash 도구로 `vp task update <task_id> in_progress --json` 명령을 실행하세요. 상태 변경
   ...
   **c. 인라인 검증**
   서브에이전트를 사용하지 않으므로, verification 스킬의 검증 절차를 직접 수행하세요:

   - **테스트 실행**: 프로젝트의 테스트 명령 실행 (package.json의 test 스크립트, 또는 pytest, cargo test 등 프로젝트에 맞는 명령)
   ...
   판정 기준:
   ```
   PASS = 기술 검증 통과 AND acceptance 전항목 충족
   WARN = 기술 검증 통과 AND acceptance에 미확인 항목 존재
   FAIL = 기술 검증 실패 OR acceptance 미충족 항목 존재
   ```

   **d. 셀프 코드 리뷰**
   codex-review를 대신하여, `git diff`로 현재 태스크의 변경분을 확인하고 점검하세요:
   - 보안 취약점 (인젝션, 하드코딩된 시크릿, 미검증 입력)
   - 명백한 버그 (off-by-one, null 참조, 리소스 미해제)
   - spec과의 불일치

   자신이 방금 작성한 코드를 리뷰하는 것이므로 편향될 수 있습니다. "내가 놓쳤을 수 있는 것"을 의식적으로 찾으세요.

   **e. 태스크 완료 처리**
   - PASS: Bash 도구로 `vp task update <task_id> done --json` 명령을 실행하세요 — 다음 태스크로 진행
   - WARN: 간략히 사용자에게 알리고 Bash 도구로 `vp task update <task_id> done --json --has-concerns` 명령을 실행하세요. 기록 후 계속 진행 (최종 리포트에서 모아서 보고)
   - FAIL: 1회 자동 수정 시도 → 재검증 → 여전히 FAIL이면 blocked 처리하고 사용자에게 보고

   **f. 진행 저장**
   - 3개 태스크마다 또는 FAIL 발생 시 Bash 도구로 `vp context save --json --summary "..."` 명령을 실행하세요. 진행 상황 저장
...
- **서브에이전트 사용 금지**: tdd-implementer, codex-review 등의 에이전트를 디스패치하지 마세요. 이 스킬의 존재 이유가 서브에이전트 없이 동작하는 것입니다.
