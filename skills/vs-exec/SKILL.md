...
2. **활성 플랜 확인**
   - `vs_plan_list`로 active/approved 플랜을 조회하세요
   - 여러 개면 `AskUserQuestion`으로 선택받으세요
   - 없으면 `/vs-plan`으로 안내하세요

3. **플랜 전체 로드**
   - `vs_plan_get(plan_id)`으로 spec + 전체 태스크 트리를 가져오세요
...
8. **태스크 루프**
   ...
   **a. 태스크 시작**
   ...
   - `vs_task_update(task_id, 'in_progress')`로 상태 변경
