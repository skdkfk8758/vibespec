...
1. **대시보드 데이터 조회**
   - Bash 도구로 `vs dashboard --json` 명령을 실행하세요. 활성 플랜 현황과 알림을 가져오세요
...
3. **속도 통계**
   - Bash 도구로 `vs stats --json` 명령을 실행하세요. 일일 완료 속도를 보여주세요
   - 활성 플랜이 있으면 Bash 도구로 `vs stats <plan_id> --json` 명령을 실행하세요. 예상 완료일도 보여주세요
