...
8. **골격 문서 건강도** (선택적)
   ...
   프로젝트 루트에서 4종 골격 문서(PRD.md, DESIGN.md, POLICY.md, ARCHITECTURE.md)를 스캔하세요.
   - 1개 이상 존재하면 건강도 요약 1줄을 표시:
     ```
     ### 골격 건강도
     PRD(85) DESIGN(60) POLICY(❌) ARCH(40) — 평균 46/100
     ```
   - 건강도 = (해당 문서의 필수섹션 존재 수 / 전체 필수섹션 수) × 100. 미존재 문서는 0점.
   - 평균 80 미만이면 경고: "⚠️ 골격 건강도가 낮습니다. `/vs-skeleton-init`으로 보완하세요."
   - 골격 문서가 하나도 없으면 이 섹션을 생략하세요.
   - 상세 확인은 `/vs-skeleton-status`로 안내하세요.
