...
3. 리뷰 결과를 context_log에 저장:
   - **FAIL 판정 시**: `vs context save --summary "[code-review-fail] Critical {N} / High {N} — 미해결 이슈 있음"`
   - **PASS/WARN 판정 시**: `vs context save --summary "[code-review] {판정}: Critical {N} / High {N} / Medium {N} / Auto-Fixed {N}"`
   - `[code-review-fail]` 태그는 vs-commit이 참조하여 경고를 표시합니다
