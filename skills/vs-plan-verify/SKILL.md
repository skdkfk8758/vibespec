...
1. **플랜 상태 조회**
   - plan_id가 없으면 `vp_plan_list(status=active)` → `vp_plan_list(status=approved)` 순으로 탐색하세요
   - 복수 플랜이면 사용자에게 선택을 요청하세요
   - `vp_plan_get(plan_id)`로 플랜 + 태스크 트리를 조회하세요
...
7. **판정별 후속 처리**
   ...
   **PASS:**
   ...
   - 선택지:
     - "플랜 완료" → `vp_plan_complete(plan_id)` 실행
     - "머지 진행" → `/vs-merge` 안내
     - "유지" → 아무 작업 안 함
   ...
   **FAIL:**
   ...
   - 선택지:
     ...
     - "강제 완료" → `vp_plan_complete(plan_id)` 실행 (경고 포함)
     - "유지" → 아무 작업 안 함
...
- `vp_plan_complete`는 사용자 승인 후에만 호출하세요
