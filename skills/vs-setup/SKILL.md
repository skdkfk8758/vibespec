---
name: vs-setup
description: [Util] Use when performing initial VibeSpec setup and SDD onboarding.
invocation: deferred
---
...
2. **기존 데이터 확인**
   - 대시보드 결과에 기존 플랜이 있으면:
     → "기존 플랜이 있습니다. `/vs-recap`으로 이전 세션을 복원할 수 있습니다." 안내
   - 기존 플랜이 없으면 다음 단계로 진행

3. **SDD 워크플로우 설명**
   사용자에게 다음을 설명하세요:
   - VibeSpec은 **Spec → Plan → Tasks → Implementation** 사이클을 따릅니다
   - 매 세션 시작 시 `/vs-recap`으로 이전 세션 리포트를 복원합니다
...
다음 세션 시작 시에는 `/vs-recap`을 실행하여 이전 세션 리포트를 복원하세요.
