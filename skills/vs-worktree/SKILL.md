...
2. **워크트리 이름 결정**
   - 활성 플랜이 있으면 (Bash 도구로 `vs plan list --json`을 실행하여 확인): 플랜 제목에서 kebab-case 이름을 자동 생성하세요
...
8. **가드레일 제안**
   - 워크트리 생성이 완료되면 `AskUserQuestion`으로 안내하세요:
     - "워크트리 디렉토리로 편집 범위를 제한(freeze)하시겠습니까?"
     - 선택지: "활성화" (→ `vs freeze set <worktree_path>`), "건너뛰기"
   - 이 제안은 선택적이며, 사용자가 건너뛰면 제한 없이 진행합니다
