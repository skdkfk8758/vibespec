...
1. **요구사항 인터뷰**
   ...
   **체크포인트**: 아래 형식으로 요약을 보여준 뒤 `AskUserQuestion`으로 확인받으세요:
   ...
   - 선택지:
     ...
     - label: "비판적 검토 요청", description: "Claude가 요구사항을 비판적으로 검토하여 개선점을 제안합니다"

   "수정할 부분이 있습니다"를 선택하면 피드백을 반영하여 요약을 갱신하고 다시 확인받으세요.

   "비판적 검토 요청"을 선택하면 아래 가이드라인에 따라 요구사항을 비판적으로 검토하세요:
   ...
   **검토 결과 출력** (심각도순 정렬, 최대 10항목):
   ```
   | # | 관점 | 심각도 | 항목 | 권장 개선 방향 |
   |---|------|--------|------|----------------|
   | 1 | 모호성 | 개선필요 | "사용자 정보를 표시한다"가 테스트 불가 | "사용자 이름, 이메일, 가입일을 표시한다"로 구체화 |
   ```

   **검토 결과 처리**:
   - 발견 항목이 0건이면: "요구사항이 잘 정리되어 있습니다. 스펙 작성으로 진행합니다."로 안내하고 Step 2로 진행하세요.
   - 발견 항목이 1건 이상이면 `AskUserQuestion`으로 처리 방식을 선택받으세요:
     - question: "검토 결과를 어떻게 반영할까요?"
     - header: "검토 반영"
     - multiSelect: false
     - 선택지:
       - label: "전체 반영", description: "모든 개선 항목을 요구사항에 반영합니다"
       - label: "선택 반영", description: "항목별로 수락/거절을 선택합니다"
       - label: "무시", description: "검토 결과를 반영하지 않고 스펙 작성으로 진행합니다"
     - "선택 반영" 시 `AskUserQuestion`으로 multiSelect: true를 사용하여 각 항목별 수락/거절을 선택받으세요:
       - 각 검토 항목을 선택지로 변환: label은 "#{번호} {관점}: {항목 요약}", description은 "{권장 개선 방향}"
       - 선택된 항목만 요구사항에 반영하세요

   **반영 후 루프**:
   - 개선점 반영 후 요구사항 요약을 갱신하고 Step 1c 확인 체크포인트로 복귀하세요
   - "비판적 검토 요청" 선택이 2회 반복되면: "추가 검토는 스펙 작성 후에도 가능합니다. 스펙 작성으로 진행하시겠습니까?"로 안내하세요
