...
1. **에러 KB 검색**
   - Bash 도구로 `vp error-kb search "<에러 키워드>" --json` 명령을 실행하세요
...
3. **에러 기록/업데이트**
   - 새 에러인 경우:
     - Bash 도구로 `vp error-kb add --title "..." --cause "..." --solution "..." --tags "tag1,tag2" --severity <level> --json` 명령을 실행하세요
   - 기존 에러가 재발한 경우:
     - Bash 도구로 `vp error-kb update <id> --occurrence "재발 상황 설명" --json` 명령을 실행하세요
...
- **vs-next**: 태스크 시작 시 관련 키워드로 `vp error-kb search` 실행을 권장합니다. 사전에 알려진 에러 패턴을 파악하면 구현 품질이 향상됩니다.
- **systematic-debugging**: 디버깅 스킬을 사용하여 문제를 해결한 후 `vp error-kb add`로 원인과 해결책을 기록하세요. 향후 동일 에러 발생 시 빠르게 대응할 수 있습니다.
...
- 에러 현황 전체 조회 → `vp error-kb search "" --json`으로 전체 목록 확인
